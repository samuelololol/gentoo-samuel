...
if [[ $(get_major_version ) -ge 7 ]] ; then
	...
	#fi
	if [[ ${MY_PN} == gvim ]] ; then
		IUSE="${IUSE} netbeans aqua nextaw gdb"
		...
		RDEPEND="${RDEPEND} gdb? ( sys-devel/gdb )"
		PDEPEND="${PDEPEND} gdb? ( app-vim/vimgdb )"
	fi
	if [[ ${MY_PN} == vim ]] ; then
		IUSE="${IUSE} vim-pager gdb"
		RDEPEND="${RDEPEND} gdb? ( sys-devel/gdb )"
		PDEPEND="${PDEPEND} gdb? ( app-vim/vimgdb )"
	fi
...
vim_src_unpack() {
	...
	if [[ "${MY_PN}" == "gvim" ]] && [[ $(get_major_version ) -ge 7 ]] \
		&& use gdb ; then
		epatch ${WORKDIR}/vimgdb/vim72.diff
	fi
